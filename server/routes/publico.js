...
router.get('/partidos/:id', asyncHandler(async (req, res) => {
  ...
  const { rows: goles } = await pool.query(
    `SELECT g.id, g.equipo_id, g.jugador_id, g.minuto, g.minuto_adicion, g.tiempo, g.en_propia_puerta, g.creado_en, j.nombre AS jugador_nombre, COALESCE(pn.numero, j.numero_camiseta) AS jugador_numero
     ...
     WHERE g.partido_id = $1 ORDER BY g.tiempo NULLS LAST, g.minuto NULLS LAST, g.creado_en`,
    [req.params.id]
  );
  const { rows: tarjetas } = await pool.query(
    `SELECT t.id, t.equipo_id, t.jugador_id, t.minuto, t.minuto_adicion, t.tiempo, t.tipo, t.creado_en, j.nombre AS jugador_nombre, COALESCE(pn.numero, j.numero_camiseta) AS jugador_numero
     ...
     WHERE t.partido_id = $1 ORDER BY t.tiempo NULLS LAST, t.minuto NULLS LAST, t.creado_en`,
    [req.params.id]
  );
  const { rows: cambios } = await pool.query(
    `SELECT c.id, c.equipo_id, c.minuto, c.minuto_adicion, c.tiempo, c.creado_en,
     ...
     WHERE c.partido_id = $1 ORDER BY c.tiempo NULLS LAST, c.minuto NULLS LAST, c.creado_en`,
    [req.params.id]
  );
